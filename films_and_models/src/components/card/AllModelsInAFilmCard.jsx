...
const AllModelsInAFilmCard = () => {
    ...
    const filmData = async () => {
      try{
        ...
        setLoading(true);
        console.log("nestedCharacterUrl", nestedCharacterUrl);
        // getIdFromUrl(nestedCharacterUrl[0].url);
      }catch(err){
          console.error(err)
      }
    };
  ...
  return (
    ...
    {Loading ? ( 
          <div className="container-fluid col-xs-12 col-lg-8 mt-2 min-vh-100 allModelsInAFilmContainer">
            <div className="row pb-2 allModelsInAFilmTitle">
                <div className="col-xs-12 col-lg-6 mx-auto">
                    <h3 className="d-flex align-item-center justify-content-center">Title:<span className="ms-3">{JSON.parse(JSON.stringify(title))}</span></h3>
                </div>
              </div>
            ...
            <div className="col-xs-12 col-lg-6 my-4 mx-auto">
                  <Card className="text-bg-light mb-5">
                      ...
                      <Card.Subtitle className="mb-3 text-muted"><span className="ms-3">{spec.map((m) => <h5>
                        Model Name: <Link to ={`/models/${getIdFromUrl(m.url)}`} state = {{url: `https://www.swapi.tech/api/people/${getIdFromUrl(m.url)}`}} className="text-decoration-none ms-3">{m.name}</Link> </h5>)}</span></Card.Subtitle>
